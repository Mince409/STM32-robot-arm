...
#include "stdarg.h"
#include "OLED.h"
uint16_t RxTime[13]; //存储接收的数据 根据实际情况更改 [这个数组负责接收所有的数据]
uint8_t RxAddr;
uint16_t MySize;
uint8_t RxData;
...
void USART2_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART2,USART_IT_RXNE) == SET)
	{
		...
		//数据传入
		else if(Rxpack == 1) //正式输入传入数组
		{
			RxTime[pRxpack] = RxData;
			pRxpack++;
			if(pRxpack >= 11) //总共有多少数据11就改成多少[如何用蓝牙调试器记得把校验位也算上,其他的控制器就不知道了]
			{
				Rxpack = 2;
			}
		}
